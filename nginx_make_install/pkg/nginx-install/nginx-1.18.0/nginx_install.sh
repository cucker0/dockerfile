...
# 软件包
#  nginx-1.18.0.tar.gz LuaJIT-2.0.5.tar.gz openssl-1.1.1w.tar.gz pcre-8.44.tar.gz ngx_devel_kit-0.3.1.tag.gz lua-nginx-module-0.10.19.tar.gz ngx_healthcheck_module_1.16+.tar.gz ngx_dynamic_upstream-0.1.6.tar.gz init.d.nginx nginx_install.sh
...
function before_nginx_install() {
    ...
    tar -zxvf ngx_devel_kit-0.3.1.tag.gz
    tar -zxvf ngx_dynamic_upstream-0.1.6.tar.gz

    # 打上nginx_upstream_check_module 补丁
    tar -zxvf ngx_healthcheck_module_1.16+.tar.gz
    tar -zxvf nginx-${NGINX_VERSION}.tar.gz
    cd nginx-${NGINX_VERSION}
    patch -p1 < ${workdir}/ngx_healthcheck_module_1.16+/nginx_healthcheck_for_nginx_1.14+.patch
...
function nginx_install() {
    ...
    ./configure --prefix=/usr/local/nginx_${NGINX_VERSION} --user=nginx --group=nginx --with-http_stub_status_module --with-http_ssl_module --with-pcre=${workdir}/pcre-${PCRE_VERSION} --with-http_realip_module --with-http_image_filter_module --with-http_gzip_static_module --with-openssl=${workdir}/openssl-${OPENSSL_VERSION} --add-module=${workdir}/ngx_devel_kit-0.3.1 --add-module=${workdir}/lua-nginx-module-0.10.19 --add-module=${workdir}/ngx_healthcheck_module_1.16+ --add-module=${workdir}/ngx_dynamic_upstream-0.1.6 --with-stream --with-stream_ssl_module --with-http_v2_module
